...
estudos com progresso por matéria, metas de calendário, sincronização entre aparelhos por QR/código,
personalização visual (tema, papel de parede, fonte, estilo dos cartões), e a Agnes — companheira de
estudos conversacional (opcional, precisa do backend local rodando — veja `backend/README.md`).
...
2. Envie **todos os arquivos desta pasta** para a raiz do repositório (Add file → Upload files),
   mantendo os nomes exatamente como estão — sem colocar em subpasta (exceto a pasta `backend/`, que
   pode ir junto do jeito que está).
...
- `backend/` — servidor local opcional que dá vida à Agnes (conversa, memória, ferramentas). O app
  funciona 100% sem ele — só a bolha de conversa fica sem resposta. Veja `backend/README.md`.
...
- **Agnes** (bolha ✨ no canto da tela) só responde quando o backend local (`backend/`) está rodando na
  mesma máquina do navegador — veja `backend/README.md` para configurar. Sem o backend, a bolha aparece
  normalmente mas avisa que não conseguiu conectar.
